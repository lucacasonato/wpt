...
class BidiSession:
    ...
    :param capabilities: Capabilites response of existing session
    ...
    def __init__(self,
        ...
        if session_id is None and capabilities is not None:
            raise ValueError("Tried to create BiDi-only session with existing capabilites")
    ...
    async def start(self) -> None:
        ...
        if self.session_id is None:
            self.capabilities = await self.session.new(self.requested_capabilities)
